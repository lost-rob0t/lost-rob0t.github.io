:PROPERTIES:
:ID:       15039615-8359-410b-a6a5-ff05f3e39465
:END:
#+TITLE: OnlyForYou
#+SETUPFILE: /home/unseen/Documents/Notes/org/roam/org-setup.org

#+CREATED: [2023-04-27 Thu 21:22]
#+LAST_MODIFIED: [2023-05-26 Fri 21:51]
...
When the ~os.path.join()~ function receives a full file path, any path parts that come before the full path will be ignored.[fn:5]
...
Immediately, I can see that when checking the function ~issecure~, it runs dig with the ~shell=True~ option. This allows for command injection, which can be done by giving an email and then ~;<command>~, which will pass the email regex and then run the dig command.[fn:6]
...
I'll leave the exercise of uploading a tool to the target host as an exercise for the reader.[fn:2]  Once I got LinPEAS.sh onto the host, I started with some basic enumeration.
...
I use chisel since that is what was recommended to me.[fn:4]

**** Neo4j ports
By looking up some of the ports, we find that it is neo4j (you might already know that from the passwd file).
This table is from the Documentation for neo4j.[fn:3]

| Name                         | Default port | Related configuration setting                                                              |
|------------------------------+--------------+--------------------------------------------------------------------------------------------|
| Backup                       | ~6362~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.backup.listen_address][server.backup.listen_address]]                                                               |
| HTTP                         | ~7474~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.http.listen_address][server.http.listen_address]]                                                                 |
| HTTPS                        | ~7473~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.https.listen_address][server.https.listen_address]]                                                                |
| Bolt                         | ~7687~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.bolt.listen_address][server.bolt.listen_address]]                                                                 |
| Cluster discovery management | ~5000~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.discovery.listen_address][server.discovery.listen_address]]                                                            |
| Cluster transaction          | ~6000~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.cluster.listen_address][server.cluster.listen_address]]                                                              |
| Cluster RAFT                 | ~7000~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.cluster.raft.listen_address][server.cluster.raft.listen_address]]                                                         |
| Cluster routing connector    | ~7688~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.routing.listen_address][server.routing.listen_address]]                                                              |
| Graphite monitoring          | ~2003~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.metrics.graphite.server][server.metrics.graphite.server]]                                                             |
| Prometheus monitoring        | ~2004~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.metrics.prometheus.endpoint][server.metrics.prometheus.endpoint]]                                                         |
| JMX monitoring               | ~3637~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.jvm.additional][server.jvm.additional=-Dcom.sun.management.jmxremote.port=3637]]                             |
| Remote debugging             | ~5005~         | [[https://neo4j.com/docs/operations-manual/current/reference/configuration-settings/#config_server.jvm.additional][server.jvm.additional=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005]] |
...
I came to realize this when fuzzing the search page for [[id:81d62666-4ce8-4244-b498-739a680f9673][SQL Injection]].
Cypher injections involve a form of SSRF, which is how you can get the data in this case. You will need to have an HTTP server running where you can see the requests. [fn:2]
...
[fn:7] https://book.hacktricks.xyz/pentesting-web/sql-injection/cypher-injection-neo4j

[fn:6] https://stackoverflow.com/questions/3172470/actual-meaning-of-shell-true-in-subprocess

[fn:5] https://docs.python.org/3/library/os.path.html#os.path.join
[fn:4] https://github.com/jpillora/chisel

[fn:3] https://neo4j.com/docs/operations-manual/current/configuration/ports/

[fn:2] My advice is to have a directory for the tools then use ~python -m http.server~ or pwncat to download it
